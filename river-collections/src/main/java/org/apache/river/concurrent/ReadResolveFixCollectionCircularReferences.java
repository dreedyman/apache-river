 ...
 */
public abstract class ReadResolveFixCollectionCircularReferences<T> extends SerializationOfReferenceCollection<T>
implements List<T>, Set<T>, SortedSet<T>, NavigableSet<T>, Queue<T>, Deque<T>, BlockingQueue<T>, BlockingDeque<T>{
   
    // This abstract class must not hold any serial data.

	// a hacky workaround, for now, to deal with the "Java8 and above" error that you get when you
	// try to implement BOTH List and SortedSet.  See:
